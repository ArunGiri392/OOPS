...
class OpenAccount(Transaction):
    ...
    def get_transaction_description(self):
        return f'Teller {self.get_teller_id()} opened account for {self.get_customer_id()}'
...
class BankBranch:
    ...
    def _get_avaialable_tellers(self):
        index = round(random.random() * (len(self.tellers) - 1))
        return self.tellers[index].get_id()
    
    def open_account(self, customer_name):
        if not self.tellers:
            raise ValueError('Branch does not have any tellers now')
        teller_id = self._get_avaialable_tellers()
...
class Bank:
    def __init__(self, bank_name, branches, bank_system, total_cash):
        self._bank_name = bank_name
        self._branches = branches
        self._banksystem = bank_system
        self._total_cash = total_cash 
    
    def add_branch(self, address, initial_funds):
        branch = BankBranch(address, initial_funds, self._banksystem)
...
bank = Bank("chase", [], bankSystem, 10000)
